...
router.put("/matrix", async (req, res) => {
    ...
    const ch_matrix = await mongoAppClient.db.collection("storage").findOne({ "name": body.name });
    if (ch_matrix) {
...
router.post("/matrix/:id", async (req, res) => {
    const matrix = new Matrix();
    let fstmx = await mongoAppClient.db.collection("matrix_create").findOne({ "_id": new mongoose.Types.ObjectId(req.params.id) });
    const page = req.query.p ?? 1;
    const body = req.body;
    if (fstmx) {
        const pages = Math.ceil(await matrix.getMatrixDocs(fstmx.name) / offset);
...
router.post("/storage/matrix/:id", async (req, res) => {
    const obj = storage.getMatrixList(req.params.id);
