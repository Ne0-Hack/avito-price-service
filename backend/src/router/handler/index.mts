...
type PriceHandlerBody = {
    "location_id": number,
    "microcategory_id": number,
    "user_id": number
}

const searchUser = async (lc: number, mc: number, user_id: number) => {
...
router.post("/getprice", async (req: any, res: any) => {
        const body: PriceHandlerBody = req.body
        const errors = []
        if(!body.location_id) {
